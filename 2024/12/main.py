...
def coords_to_direction(row: int, col: int) -> str:
    if row == 0:
        if col == -1:
            return 'left'
        else:
...
def get_value(item: Tuple[int, int, str]) -> str:
    return item[2]


def count_corners(cell: str, neighbors: Dict[str, Tuple[int, int, str]]) -> int:
    corners: int = 0
    if 'up' in neighbors and 'right' in neighbors and 'right-up' in neighbors:
        # XO
        # XX
        if get_value(neighbors['up']) == cell and get_value(neighbors['right']) == cell and get_value(
            neighbors['right-up']
        ) != cell:
            corners += 1
        # OX
        ...
        if get_value(neighbors['up']) == cell and get_value(neighbors['right']) != cell and get_value(
            ...
            corners += 1
    # if 'up' in neighbors and 'right' in neighbors and 'right-down' in neighbors:
    #     # X
    #     # XX
    #     if get_value(neighbors['up']) == cell and get_value(neighbors['right']) == cell and get_value(neighbors['right-down']) != cell:
    #         corners += 1
    #     if get_value(neighbors['up']) != cell and get_value(neighbors['right']) == cell and get_value(neighbors['right-down']) == cell:
    #         corners += 1
    #     if get_value(neighbors['up']) == cell and get_value(neighbors['right']) != cell and get_value(neighbors['right-down']) == cell:
    #         corners += 1
    # if 'down' in neighbors and 'right' in neighbors and 'right-up' in neighbors:
    #     if get_value(neighbors['down']) == cell and get_value(neighbors['right']) == cell and get_value(neighbors['right-up']) != cell:
    #         corners += 1
    #     if get_value(neighbors['down']) != cell and get_value(neighbors['right']) == cell and get_value(neighbors['right-up']) == cell:
    #         corners += 1
    #     if get_value(neighbors['down']) == cell and get_value(neighbors['right']) != cell and get_value(neighbors['right-up']) == cell:
    #         corners += 1
    # if 'down' in neighbors and 'right' in neighbors and 'right-down' in neighbors:
    #     if get_value(neighbors['down']) == cell and get_value(neighbors['right']) == cell and get_value(neighbors['right-down']) != cell:
    #         corners += 1
    #     if get_value(neighbors['down']) != cell and get_value(neighbors['right']) == cell and get_value(neighbors['right-down']) == cell:
    #         corners += 1
    #     if get_value(neighbors['down']) == cell and get_value(neighbors['right']) != cell and get_value(neighbors['right-down']) == cell:
    ...
    if 'up' in neighbors and 'left' in neighbors and 'left-up' in neighbors:
        # OX
        # XX
        if get_value(neighbors['up']) == cell and get_value(neighbors['left']) == cell and get_value(
            neighbors['left-up']
        ) != cell:
            corners += 1
        # XO
        ...
        if get_value(neighbors['up']) == cell and get_value(neighbors['left']) != cell and get_value(
            ...
            corners += 1
    # if 'up' in neighbors and 'left' in neighbors and 'left-down' in neighbors:
    #     if get_value(neighbors['up']) == cell and get_value(neighbors['left']) == cell and get_value(neighbors['left-down']) != cell:
    #         corners += 1
    #     if get_value(neighbors['up']) != cell and get_value(neighbors['left']) == cell and get_value(neighbors['left-down']) == cell:
    #         corners += 1
    #     if get_value(neighbors['up']) == cell and get_value(neighbors['left']) != cell and get_value(neighbors['left-down']) == cell:
    #         corners += 1
    # if 'down' in neighbors and 'left' in neighbors and 'left-up' in neighbors:
    #     if get_value(neighbors['down']) == cell and get_value(neighbors['left']) == cell and get_value(neighbors['left-up']) != cell:
    #         corners += 1
    #     if get_value(neighbors['down']) != cell and get_value(neighbors['left']) == cell and get_value(neighbors['left-up']) == cell:
    #         corners += 1
    #     if get_value(neighbors['down']) == cell and get_value(neighbors['left']) != cell and get_value(neighbors['left-up']) == cell:
    #         corners += 1
    # if 'down' in neighbors and 'left' in neighbors and 'left-down' in neighbors:
    #     if get_value(neighbors['down']) == cell and get_value(neighbors['left']) == cell and get_value(neighbors['left-down']) != cell:
    #         corners += 1
    #     if get_value(neighbors['down']) != cell and get_value(neighbors['left']) == cell and get_value(neighbors['left-down']) == cell:
    #         corners += 1
    #     if get_value(neighbors['down']) == cell and get_value(neighbors['left']) != cell and get_value(neighbors['left-down']) == cell:
...
def get_neighbors(position: Tuple[int, int], matrix: List[List[str]]) -> Dict[
    str, Tuple[int, int, str]]:
    row, col = position
    neighbors: Dict[str, Tuple[int, int, str]] = {}
    for x in [-1, 0, 1]:
        for y in [-1, 0, 1]:
            if x == 0 and y == 0:
                continue
            ...
            if row + x >= len(matrix) or col + y >= len(matrix[row + x]):
                continue
            try:
                neighbors[coords_to_direction(x, y)] = (row + x, col + y, matrix[row + x][col + y])
            except IndexError as _:
                pass
    return neighbors
